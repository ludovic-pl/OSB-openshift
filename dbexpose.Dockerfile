...
COPY --from=$DBDATA --chown=$USER:$GROUP /neo4j/data/backup /osb/backup
...
ENV NEO4J_AUTH=neo4j/${neo4jpwd} \
    ...
    NEO4J_apoc_initializer_system_1="CREATE DATABASE mdrdb OPTIONS {existingData: 'use', seedURI:'file:///osb/backup/mdrdockerdb.backup'} WAIT 60 SECONDS"
